...
my $outdir = "/leofs/wulin_group/zhangcm/panqf/omics/01_PGA/03_variant_peptides";
my @sample = qw/1N 2N 3N 4N 5N 1T 2T 3T 4T 5T/;

for(my$i=0;$i<@sample;$i++) {
    open (IN, "$indir/PGA_$sample[$i]_peptides.txt") or die;
    open (OUT, "> $outdir/PGA_$sample[$i]_variant_peptides.txt") or die;
