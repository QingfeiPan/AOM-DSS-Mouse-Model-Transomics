...
open (IN1, "$dir/12_abundance_foldchange_mrna.txt") or die;
my %fpkm = ();
while (<IN1>) {
    chomp;
    if ($_ =~ /^Gene/) {
...
open (IN2, "$dir/12_abundance_foldchange_protein_UR.txt") or die;
my %lfq = ();
while (<IN2>) {
    chomp;
    if ($_ =~ /^Gene/) {
...
open (OUT, "> $dir/13_abundance_summary_UR.txt") or die;
my ($line1, $line2);
